/*
  # Supprimer les espaces des références

  1. Modification
    - Mise à jour de la fonction generate_property_reference pour supprimer tous les espaces
    - Les références seront collées sans espaces
    
  2. Notes
    - Utilisation de REPLACE() pour supprimer les espaces
    - Application à toutes les parties de la référence
*/
...
BEGIN
  ...
  WHILE v_result ~ '\{owner_name:\d+\}' LOOP
    ...
    v_result := regexp_replace(v_result, '\{owner_name:\d+\}', UPPER(REPLACE(LEFT(COALESCE(p_owner_name, ''), v_length), ' ', '')), '');
  END LOOP;

  WHILE v_result ~ '\{owner_first_name:\d+\}' LOOP
    v_length := (regexp_match(v_result, '\{owner_first_name:(\d+)\}'))[1]::INTEGER;
    v_result := regexp_replace(v_result, '\{owner_first_name:\d+\}', UPPER(REPLACE(LEFT(COALESCE(p_owner_first_name, ''), v_length), ' ', '')), '');
  END LOOP;

  WHILE v_result ~ '\{address:\d+\}' LOOP
    v_length := (regexp_match(v_result, '\{address:(\d+)\}'))[1]::INTEGER;
    v_result := regexp_replace(v_result, '\{address:\d+\}', UPPER(REPLACE(LEFT(COALESCE(p_address, ''), v_length), ' ', '')), '');
  END LOOP;

  WHILE v_result ~ '\{type:\d+\}' LOOP
    v_length := (regexp_match(v_result, '\{type:(\d+)\}'))[1]::INTEGER;
    v_result := regexp_replace(v_result, '\{type:\d+\}', UPPER(REPLACE(LEFT(COALESCE(p_type, ''), v_length), ' ', '')), '');
  END LOOP;
